...
class ImageClassify(object):
    ...
    def updata_dataloader(self, i):
        print("========================= New Training ===========================")
        # print(" Training dataset: {}".format(train_city_index))
        # print(" Testing dataset:  {}".format(test_city_index))
        self.train_dataset = dataloader.Load_Data(data_root=config["data_dir"],
                                                  json_root=config["json_root"].format(i), type="train")
        ...
        self.valid_dataset = dataloader.Load_Data(data_root=config["data_dir"],
                                                  json_root=config["json_root"].format(i), type="val")
        ...
        self.model = model.Net(dim=data_train_opt["dim"], city_class_num=10, scene_class_num=10,
                               K=data_train_opt["K"])
    ...
    def TrainingData(self):
        ...
        for epoch in range(data_train_opt['epoch']):
            ...
            for i, (sample, city,scene,device,sample_t, city_t,scene_t,device_t) in enumerate(self.trainloader):
                ...
                for n in range(1):
                    pre_device,pre_city, f_1, f_2, t_1, t_2 = self.model(x=sample,x_t=sample_t,type="train",alpha=alpha)
                    pre_device = pre_device[int(batch/2):]
                    pre_city = pre_city[int(batch/2):]
                    all_device_label = all_device_label[int(batch/2):]
                    all_city_label = all_city_label[int(batch/2):]
                    loss_d = self.criterion(pre_device,all_device_label)
                    # loss_c = self.criterion(pre_city[:batch+int(batch/4)],all_city_label[:batch+int(batch/4)])

                    joint_pre_device = pre_device.unsqueeze(2)
                    joint_pre_city = pre_city.unsqueeze(1)
                    pre_city = joint_pre_device.bmm(joint_pre_city).view(pre_device.size(0), -1)
                    ...
                    loss = data_train_opt["mcd"] * torch.mean(torch.sum(torch.abs(t_1 - t_2), dim=-1)) + (
                                self.criterion(f_1, scene) + self.criterion(f_2, scene)) + data_train_opt["grl"] * (loss_d+self.criterion(pre_city,all_city_label))
            ...
            if (epoch+1) % data_train_opt["save_epoch"] == 0:
                ...
                if acc > self.best:
                    self.best = acc
                    a = 1
                acc_add = data_train_opt["acc_log"].format(self.fold)
                ...
                if a == 1:
                    ...
                    }, filename=os.path.join(data_train_opt["feat_training_file"],'best_model_{}.pth'.format(self.fold)))
    ...
    def all_training_plan(self):

        for fold in range(5):
            self.best = 0
            self.loss = 9999
            self.fold = fold
            self.updata_dataloader(fold)
            self.TrainingData()

def main():

    ImgCla = ImageClassify()
    ImgCla.all_training_plan()
